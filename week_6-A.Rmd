...
# sample 30 beads at random 5 times
# replicate 1:5
# samp: list-columns
...
```{r}
# more complicated way of doing it: 
scene_2 <- function(size = 50){
sample_n(urn, size = size) %>%
  summarize(is_red = (color == "red"),
            is_blue = (color == "blue")) %>%
  summarize(num_red = sum(is_red),
            num_blue = sum(is_blue)) %>%
  summarize(more_red = (num_red > num_blue)) 
}

prob_more_red_than_blue <- tibble(replicate = 1:100,
samp = map(replicate, ~ scene_2(size = 50), replace = TRUE)) %>%
  unnest(samp) %>%
  summarize(prob = mean(more_red))

prob_more_red_than_blue
...
scene_3 <- tibble(numb_red = rep(seq(0, urn_size, 1), 100)) %>%
mutate(red_paddle = map_int(numb_red, ~ rbinom(n = 1, size = 25, 
                                                 prob = ./nrow(urn))))
